...
pure subroutine XDERIV(lon, input, output, periodic, status)
    ...
    if (work_periodic) then
        ...
        coef(1)    = real(1, kind=RK) / (dlon(1)*dlon(2) * (dlon(1) + dlon(2)))
        do i = 2, nx-1
            work_du(i) = dlon(i+1) * dlon(i+1)
            work_dl(i) = dlon(i  ) * dlon(i  )
            coef(i)    = real(1, kind=RK) / (dlon(i)*dlon(i+1) * (dlon(i) + dlon(i+1)))
        enddo
        work_du(nx) = dlon(1 ) * dlon(1 )
        work_dl(nx) = dlon(nx) * dlon(nx)
        coef(nx)    = real(1, kind=RK) / (dlon(nx)*dlon(1) * (dlon(nx) + dlon(1)))
        ...
        work_du(1) = real(1, kind=RK) / dlon(2)
        do i = 2, nx-1
            work_du(i) = dlon(i+1) * dlon(i+1)
            work_dl(i) = dlon(i  ) * dlon(i  )
            coef(i)    = real(1, kind=RK) / (dlon(i)*dlon(i+1) * (dlon(i) + dlon(i+1)))
        enddo
        work_dl(nx) = real(1, kind=RK) / dlon(nx)
...
pure subroutine YDERIV(lat, input, output, status)
    ...
    work_du(1) = real(1, kind=RK) / dlat(1)
    do j = 2, ny-1
        work_du(j) = dlat(j  ) * dlat(j  )
        work_dl(j) = dlat(j-1) * dlat(j-1)
        coef(j)    = real(1, kind=RK) / (dlat(j-1)*dlat(j) * (dlat(j-1) + dlat(j)))
    enddo
    work_dl(ny) = real(1, kind=RK) / dlat(ny-1)
...
pure subroutine ZDERIV(lev, input, psfc, output, undef, status)
    ...
    toa_dlev_inv    = real(1, kind=RK) / (lev(top+dir) - lev(top))
    bottom_dlev_inv = real(1, kind=RK) / (lev(bottom) - lev(bottom-dir))
    ...
    do k = top+dir, bottom-dir, dir
        ...
        du   = dlev(k  ) * dlev(k  )
        coef = real(1, kind=RK) / (dlev(k-1)*dlev(k) * (dlev(k-1) + dlev(k)))
        sfc_dlev_inv = real(1, kind=RK) / (lev(k) - lev(k-dir))
        do j = 1, ny
            do i = 1, nx
                if (isAtmos(i,j)) then
